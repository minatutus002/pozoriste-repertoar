// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Pozoriste.DataAccess.Context;

#nullable disable

namespace Pozoriste.DataAccess.Migrations
{
    [DbContext(typeof(PozoristeDbContext))]
    [Migration("20260129103354_UpdateGlumciConfig")]
    partial class UpdateGlumciConfig
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.23")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Pozoriste.Models.Entities.Glumac", b =>
                {
                    b.Property<int>("GlumacId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("GlumacId"));

                    b.Property<string>("PunoIme")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("nvarchar(120)");

                    b.HasKey("GlumacId");

                    b.HasIndex("PunoIme")
                        .IsUnique();

                    b.ToTable("Glumci", (string)null);
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.Predstava", b =>
                {
                    b.Property<int>("PredstavaId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("PredstavaId"));

                    b.Property<decimal>("Cena")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Naziv")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Opis")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("SlikaUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("TrajanjeMin")
                        .HasColumnType("int");

                    b.Property<int>("ZanrId")
                        .HasColumnType("int");

                    b.HasKey("PredstavaId");

                    b.HasIndex("ZanrId");

                    b.ToTable("Predstava", (string)null);
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.PredstavaGlumac", b =>
                {
                    b.Property<int>("PredstavaId")
                        .HasColumnType("int");

                    b.Property<int>("GlumacId")
                        .HasColumnType("int");

                    b.HasKey("PredstavaId", "GlumacId");

                    b.HasIndex("GlumacId");

                    b.ToTable("PredstavaGlumci", (string)null);
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.Rezervacija", b =>
                {
                    b.Property<int>("RezervacijaId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("RezervacijaId"));

                    b.Property<int>("BrojKarata")
                        .HasColumnType("int");

                    b.Property<DateTime>("DatumKreiranja")
                        .HasColumnType("datetime2");

                    b.Property<string>("KorisnikId")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<int>("TerminId")
                        .HasColumnType("int");

                    b.Property<decimal>("UkupnaCena")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.HasKey("RezervacijaId");

                    b.HasIndex("TerminId");

                    b.ToTable("Rezervacija", (string)null);
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.RezervacijaSediste", b =>
                {
                    b.Property<int>("RezervacijaSedisteId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("RezervacijaSedisteId"));

                    b.Property<int>("Broj")
                        .HasColumnType("int");

                    b.Property<decimal>("Cena")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<int>("Red")
                        .HasColumnType("int");

                    b.Property<int>("RezervacijaId")
                        .HasColumnType("int");

                    b.Property<int>("TerminId")
                        .HasColumnType("int");

                    b.Property<string>("Zona")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("RezervacijaSedisteId");

                    b.HasIndex("RezervacijaId");

                    b.HasIndex("TerminId", "Red", "Broj")
                        .IsUnique();

                    b.ToTable("RezervacijaSediste", (string)null);
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.Sala", b =>
                {
                    b.Property<int>("SalaId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("SalaId"));

                    b.Property<int>("BrojRedova")
                        .HasColumnType("int");

                    b.Property<int>("Kapacitet")
                        .HasColumnType("int");

                    b.Property<string>("Naziv")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SedistaPoRedu")
                        .HasColumnType("int");

                    b.HasKey("SalaId");

                    b.ToTable("Sala", (string)null);
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.Termin", b =>
                {
                    b.Property<int>("TerminId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("TerminId"));

                    b.Property<DateTime>("DatumVreme")
                        .HasColumnType("datetime2");

                    b.Property<int>("PredstavaId")
                        .HasColumnType("int");

                    b.Property<int>("SalaId")
                        .HasColumnType("int");

                    b.HasKey("TerminId");

                    b.HasIndex("PredstavaId");

                    b.HasIndex("SalaId");

                    b.ToTable("Termin", (string)null);
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.Zanr", b =>
                {
                    b.Property<int>("ZanrId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ZanrId"));

                    b.Property<string>("Naziv")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ZanrId");

                    b.ToTable("Zanr", (string)null);
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.Predstava", b =>
                {
                    b.HasOne("Pozoriste.Models.Entities.Zanr", "Zanr")
                        .WithMany()
                        .HasForeignKey("ZanrId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Zanr");
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.PredstavaGlumac", b =>
                {
                    b.HasOne("Pozoriste.Models.Entities.Glumac", "Glumac")
                        .WithMany("Predstave")
                        .HasForeignKey("GlumacId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Pozoriste.Models.Entities.Predstava", "Predstava")
                        .WithMany("Glumci")
                        .HasForeignKey("PredstavaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Glumac");

                    b.Navigation("Predstava");
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.Rezervacija", b =>
                {
                    b.HasOne("Pozoriste.Models.Entities.Termin", "Termin")
                        .WithMany()
                        .HasForeignKey("TerminId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Termin");
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.RezervacijaSediste", b =>
                {
                    b.HasOne("Pozoriste.Models.Entities.Rezervacija", "Rezervacija")
                        .WithMany("Sedista")
                        .HasForeignKey("RezervacijaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Pozoriste.Models.Entities.Termin", "Termin")
                        .WithMany()
                        .HasForeignKey("TerminId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Rezervacija");

                    b.Navigation("Termin");
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.Termin", b =>
                {
                    b.HasOne("Pozoriste.Models.Entities.Predstava", "Predstava")
                        .WithMany()
                        .HasForeignKey("PredstavaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Pozoriste.Models.Entities.Sala", "Sala")
                        .WithMany()
                        .HasForeignKey("SalaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Predstava");

                    b.Navigation("Sala");
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.Glumac", b =>
                {
                    b.Navigation("Predstave");
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.Predstava", b =>
                {
                    b.Navigation("Glumci");
                });

            modelBuilder.Entity("Pozoriste.Models.Entities.Rezervacija", b =>
                {
                    b.Navigation("Sedista");
                });
#pragma warning restore 612, 618
        }
    }
}
